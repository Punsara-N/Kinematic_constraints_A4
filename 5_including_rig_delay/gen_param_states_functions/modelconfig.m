...
%% Transmission delay
if isempty(strfind(file_name,'delay')) == 0
    aa = strfind(file_name,'delay'); % Index of 'delay' in string
    bb = strfind(file_name,'_'); % Array of indexes of all '_' in string
    cc = max(find(bb < aa)); % Last index of '_' before 'delay'
    delayString = file_name(bb(cc)+1:aa-1); % Delay as a string from filename
    delay = str2num(delayString); % Converts 'delayString' string to an integer (units: milliseconds)
    FCdelay = delay/1000; % s, delay
else
    delay = 0;
    FCdelay = delay/1000; % s, delay
end
disp(['Delay = ' num2str(delay) ' ms (only when F_c is enabled)']);
