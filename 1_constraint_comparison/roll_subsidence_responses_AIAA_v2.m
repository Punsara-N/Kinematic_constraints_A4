...
%%
load('07_roll_subsidence_spherical_constrained_0_6');
...
%%
load('08_roll_subsidence_spherical_constrained_0_8');
...
%%
load('09_roll_subsidence_spherical_constrained_1_0');
...
%%
load('10_roll_subsidence_planar_constrained');
...
%             '[t1 t2 t3 t4 t5 t5_reduced t5_legend]'   	0       5       0.5         '[beta1 beta2 beta3 beta4 beta5 beta5_reduced beta5_legend]'  	-1    	1       0.5       	'$\beta$ (deg)'    	[0 0 0]   	'-|--|:|-.|-|x|-x'
%             't4'                                      	0       5       0.5         '[dail1]'                                                       -1      1       0.5        	'$\delta_a$ (deg)' 	[0 0 0]  	'-|--|:|-.|-|x|-x'
%             };
%           x                       x_min   x_max   x_spacing	y                                 	y_min   y_max   y_spacing   y_label          	Colour    	Style
plots= {    '[t1 t3 t5 t6]+1'    	1       6       0.5        	'[p1 p3 p5 p6]'                   	-10    	30  	10         	'$p$ (deg/s)'     	[0 0 0]   	'-|--|:|-.' 
            '[t1 t3 t5 t6]+1'   	1       6       0.5         '[phi1 phi3 phi5 phi6]'            	-10    	20      10         	'$\phi$ (deg)'    	[0 0 0]   	'-|--|:|-.'
            '[t1 t3 t5 t6]+1'   	1       6       0.5         '[alpha1 alpha3 alpha5 alpha6]'  	1    	2       0.5       	'$\alpha$ (deg)'  	[0 0 0]   	'-|--|:|-.'
            '[t1 t3 t5 t6]+1'   	1       6       0.5         '[beta1 beta3 beta5 beta6]'         -1    	1       0.5       	'$\beta$ (deg)'    	[0 0 0]   	'-|--|:|-.'
            't4+1'              	1       6       0.5         '[dail1]'                         	-1      1       0.5        	'$\delta_a$ (deg)' 	[0 0 0]  	'-|--|:|-.'
            };
...
%     end
    ...
    if i == 1
        ...
        axes(ha(i,:));
        rectangle('Position',[2.2 0 0.6 25]); % [X Y width height] (data units)
        % Zoomed section
        X = 0.53;
        Y = 0.89;
        W = 0.20;
        ...
        indexOfInterest = (t1 > 2.2-1) & (t1 < 2.8-1); % range of t near perturbation
        plot(t1(indexOfInterest)+1,p1(indexOfInterest),...
             t3(indexOfInterest)+1,p3(indexOfInterest),...
             t5(indexOfInterest)+1,p5(indexOfInterest),...
             t6(indexOfInterest)+1,p6(indexOfInterest)) % plot on new axes
        axis tight
        set(gca,...
            'FontUnits','points',...
            'FontWeight','normal',...
    ...
    if i == 2
       	% Create rectangle on large plot
        axes(ha(i,:));
        rectangle('Position',[2.2 2 0.6 10]); % [X Y width height] (data units)
        % Zoomed section
        X = 0.53;
        Y = 0.81;
        W = 0.20;
        ...
        box on % put box around new pair of axes
        indexOfInterest = (t1 > 2.2-1) & (t1 < 2.8-1); % range of t near perturbation
        plot(t1(indexOfInterest)+1,phi1(indexOfInterest),...
             t3(indexOfInterest)+1,phi3(indexOfInterest),...
             t5(indexOfInterest)+1,phi5(indexOfInterest),...
             t6(indexOfInterest)+1,phi6(indexOfInterest)) % plot on new axes
        axis tight
        set(gca,...
            'FontUnits','points',...
            'FontWeight','normal',...
    ...
    if i == 3
       	% Create rectangle on large plot
        axes(ha(i,:));
        rectangle('Position',[2.2 1.5 0.6 0.2]); % [X Y width height] (data units)
        % Zoomed section
        X = 0.53;
        Y = 0.72;
        W = 0.20;
        ...
        box on % put box around new pair of axes
        indexOfInterest = (t1 > 2.2-1) & (t1 < 2.8-1); % range of t near perturbation
        plot(t1(indexOfInterest)+1,alpha1(indexOfInterest),...
             t3(indexOfInterest)+1,alpha3(indexOfInterest),...
             t5(indexOfInterest)+1,alpha5(indexOfInterest),...
             t6(indexOfInterest)+1,alpha6(indexOfInterest)) % plot on new axes
        ylim([1.5 1.7]) 
        xlim([2.2 2.8])% axis tight
        set(gca,...
            'FontUnits','points',...
            'FontWeight','normal',...
            'FontSize',13,...
    ...
    if i == 4
       	% Create rectangle on large plot
        axes(ha(i,:));
        rectangle('Position',[2.2 -0.3 0.6 0.4]); % [X Y width height] (data units)
        % Zoomed section
        X = 0.53;
        Y = 0.64;
        W = 0.20;
        ...
        box on % put box around new pair of axes
        indexOfInterest = (t1 > 2.2-1) & (t1 < 2.8-1); % range of t near perturbation
        plot(t1(indexOfInterest)+1,beta1(indexOfInterest),...
             t3(indexOfInterest)+1,beta3(indexOfInterest),...
             t5(indexOfInterest)+1,beta5(indexOfInterest),...
             t6(indexOfInterest)+1,beta6(indexOfInterest)) % plot on new axes
        ylim([-0.3 0.1]) 
        xlim([2.2 2.8])% axis tight
        set(gca,...
            'FontUnits','points',...
...
indexOfInterest = (t1 <= 2.2);
indexOfInterest2 = (t1 <= 2.2);
indexOfInterest3 = (t5_reduced2 <= 2.2);
...
step = 1800;          % Gap between aircrafts on plot
exagg_factor = 5;

indexOfInterest = (t1 <= 1.8);
indexOfInterest2 = (t1 <= 1.8);
indexOfInterest3 = (t5_reduced2 <= 1.8);
...
M = trajectory3(x1(indexOfInterest),...
                ...
                'simple'); 
hold on;
xlim([-0.2 0.2]); ylim([-0.1 0.3]); zlim([-0.2 0.2]);
set(gca,...
        'Units','normalized',...
        'FontUnits','points',...
        'FontWeight','normal',...
...
% Unconstrained 2-3.2s
% Unconstrained 2-2.8s
...
print -deps RollSubsidence1.eps
print('RollSubsidence1','-dpdf','-r1000')
...
% print('RollSubsidence2','-dpdf','-loose','-painters')