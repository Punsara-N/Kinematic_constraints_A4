...
%             };
%           x                 	x_min   x_max   x_spacing	y                                	y_min   y_max   y_spacing   y_label          	Colour    	Style
plots= {    '[t1 t3 t5 t6]'   	0       8       1           '[r1 r3 r5 r6]'                  	-6      6     	3         	'$r$ (deg/s)'      	[0 0 0]   	'-|--|:|-.' 
            '[t1 t3 t5 t6]'   	0       8       1           '[p1 p3 p5 p6]'                  	-30     30      15         	'$p$ (deg/s)'       [0 0 0]   	'-|--|:|-.'
            '[t1 t3 t5 t6]'   	0       8       1           '[alpha1 alpha3 alpha5 alpha6]'  	1.5     1.8     0.1       	'$\alpha$ (deg)'   	[0 0 0]   	'-|--|:|-.'
            '[t1 t3 t5 t6]'   	0       8       1           '[beta1 beta3 beta5 beta6]'         -1      1       0.5       	'$\beta$ (deg)'    	[0 0 0]   	'-|--|:|-.'
...
%     end
    ...
    if i == 1
        ...
        axes(ha(i,:));
        rectangle('Position',[2.5 -2.7 1 6.5]); % [X Y width height] (data units)
        % Zoomed section
        X = 0.53;
        Y = 0.89;
        W = 0.20;
        ...
        plot(t1(indexOfInterest),r1(indexOfInterest),...
             t3(indexOfInterest),r3(indexOfInterest),...
             t5(indexOfInterest),r5(indexOfInterest),...
             t6(indexOfInterest),r6(indexOfInterest)) % plot on new axes
        axis tight
        set(gca,...
            'FontUnits','points',...
            'FontWeight','normal',...
    ...
    if i == 2
       	% Create rectangle on large plot
        axes(ha(i,:));
        rectangle('Position',[2.5 -5 1 30]); % [X Y width height] (data units)
        % Zoomed section
        X = 0.53;
        Y = 0.81;
        W = 0.20;
        ...
        box on % put box around new pair of axes
        indexOfInterest = (t1 > 2.5) & (t1 < 3.5); % range of t near perturbation
        plot(t1(indexOfInterest),p1(indexOfInterest),...
             t3(indexOfInterest),p3(indexOfInterest),...
             t5(indexOfInterest),p5(indexOfInterest),...
             t6(indexOfInterest),p6(indexOfInterest)) % plot on new axes
        axis tight
        set(gca,...
            'FontUnits','points',...
            'FontWeight','normal',...
    ...
    if i == 3
       	% Create rectangle on large plot
        axes(ha(i,:));
        rectangle('Position',[2.5 1.6 1 0.025]); % [X Y width height] (data units)
        % Zoomed section
        X = 0.53;
        Y = 0.72;
        W = 0.20;
    ...
    if i == 4
       	% Create rectangle on large plot
        axes(ha(i,:));
        rectangle('Position',[2.5 -0.2 1 0.6]); % [X Y width height] (data units)
        % Zoomed section
        X = 0.53;
        Y = 0.64;
        W = 0.20;
...
print -deps DutchRoll1.eps
% print('DutchRoll1','-dpdf','-loose','-painters')
print('DutchRoll1','-dpdf','-r1000')
