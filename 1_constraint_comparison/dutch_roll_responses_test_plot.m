...
figure;
subplot(4,1,1); plot(t,r); grid on; ylabel('r (deg/s)'); xlim([0 plot_time]); hold on;
subplot(4,1,2); plot(t,p); grid on; ylabel('p (deg)'); xlim([0 plot_time]); hold on;
subplot(4,1,3); plot(t,beta); grid on; ylabel('\beta (deg)'); xlim([0 plot_time]); ylim([-2 2]); hold on;
subplot(4,1,4); plot(t,drud); grid on; xlabel('Time (s)'); ylabel('\delta_r (deg)'); xlim([0 plot_time]); hold on;