...
V=V./(scale_factor*correction); % V is the 3D model
ii=length(x);
resto=mod(ii,step);
...
% % % %     i
    ...
    Tbe=[cos(psi)*cos(theta), -sin(psi)*cos(theta), sin(theta);
        ...
        cos(theta)*cos(phi)];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Vnew=V*Tbe; % Vnew is the model after rotation
    rif=[x(i) y(i) z(i)]; % Current position at i
    X0=repmat(rif,size(Vnew,1),1); % Matrix of the same positon with the size of Vnew positions
    Vnew=Vnew+X0; % Moves the rotated model to the required position
    p=patch('faces', F, 'vertices' ,Vnew);
    set(p, 'facec', [0.5 0.5 0.5]); % Colour of aircraft
