...
%% CHECK
load('03_3_short_period_phugoid_spherical_constrained_0_8_no_Fc_no_thrust_no_X_check');
...
%           x               	x_min   x_max   x_spacing	y                                 	y_min   y_max   y_spacing   y_label          	Colour    	Style
plots= {    '[t1 t2 t3 t4]'  	0       10      1           '[q1 q2 q3 q4]'                   	-30     30     	15         	'$q$ (deg/s)'      	[0 0 0]   	'-|--|:|-.|-|x|-x' 
            ...
            '[t1]'             	0       10      1           '[dele1]'                          	-4      -1      1           '$\delta_e$ (deg)' 	[0 0 0]  	'-|--|:|-.|-|x|-x'
            '[t4]'             	0       10      1           '[Fc4]'                          	-1      3       1           '$|\mathbf{F_c}|$ (N)' 	[0 0 0]  	'-|--|:|-.|-|x|-x'
            };
...
ha = tight_subplot(rows,columns,[.05 0],[.1 .05],[.15 .1]); % [gap_h gap_w], [lowermargin uppermargin], [leftmargin rightmargin]
...
figHandle = gcf;
plot1 = figHandle.Children(8);
plot2 = figHandle.Children(5);
plot3 = figHandle.Children(3);
plot4 = figHandle.Children(2);
plot5 = figHandle.Children(1);
tinyPlot1 = figHandle.Children(7);
tinyPlot2 = figHandle.Children(4);
legend = figHandle.Children(6);
...
delete(plot2); delete(tinyPlot2);
delete(plot3);
delete(plot4); % Deleting input as well, because same input as last time.

% Reposition subplots
plot5.Position = plot2Pos;
plot1.Units = 'centimeters'; % Changing units stops axes from scaling with figure size.
tinyPlot1.Units = 'centimeters';
plot5.Units = 'centimeters';
legend.Units = 'centimeters';
plot1.Position = plot1.Position + [0 -7.5 0 0];
tinyPlot1.Position = tinyPlot1.Position + [0 -7.5 0 0];
plot5.Position = plot5.Position + [0 -7.5 0 0];
legend.Position = legend.Position + [+0.5 -5.3 0 0];

% Resizing figure
currentFigPos = figHandle.Position;
figHandle.Position = currentFigPos - [0 0 0 150];
...
width = pos(3);
height = pos(4);
set(gcf,'PaperPositionMode','Auto','PaperUnits','points','PaperSize',[width,height], 'Color', 'none')
